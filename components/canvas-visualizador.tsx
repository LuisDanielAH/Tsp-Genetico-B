...
const dibujarEdificio = (ctx, x, y, size, color) => {
  const ancho = size
  const alto = size * 1.2
  ...
  for (let fila = 0; fila < 2; fila++) {
    for (let col = 0; col < 2; col++) {
      const ventX = x - ancho / 2 + espacioVentana + col * (ventanaSize + espacioVentana)
      const ventY = y - alto / 2 + espacioVentana + fila * (ventanaSize + espacioVentana)
  ...
  ctx.fillRect(x - puertaAncho / 2, y + alto / 2 - puertaAlto, puertaAncho, puertaAlto)
}
...
export default function CanvasVisualizador({
  ...
  const [modo, setModo] = useState("ciudades")
  const [ciudadSeleccionada, setCiudadSeleccionada] = useState(null)
  const [obstaculoSeleccionado, setObstaculoSeleccionado] = useState(null)
  ...
  useEffect(() => {
    ...
    obstaculos.forEach((obstaculo) => {
      ctx.fillStyle = obstaculoSeleccionado === obstaculo.id ? "#ef4444" : "#dc2626"
      ctx.beginPath()
    ...
    ciudades.forEach((ciudad, idx) => {
      const colorEdificio = ciudadSeleccionada === ciudad.id ? "#10b981" : "#0ea5e9"
  ...
  const manejarClickCanvas = (e) => {
    ...
    if (modo === "ciudades") {
      const ciudadClickeada = ciudades.find((c) => Math.hypot(c.x - x, c.y - y) < 16)
      ...
      const obstaculoClickeado = obstaculos.find((o) => Math.hypot(o.x - x, o.y - y) < o.radio + 5)
    ...
    <div className="bg-white rounded-lg shadow-lg overflow-hidden">
      ...
      <div className="p-4 bg-gradient-to-r from-indigo-50 to-blue-50 text-sm border-t border-indigo-200">
        <p className="font-semibold text-indigo-900 mb-2">
          {modo === "ciudades" ? `Ciudades: ${ciudades.length}/15` : `Obstáculos: ${obstaculos.length}`}
        </p>
        <p className="text-indigo-700">Seleccionado: {ciudadSeleccionada || obstaculoSeleccionado || "Ninguno"}</p>
        <p className="text-xs text-indigo-600 mt-1">Presiona Supr para eliminar seleccionado</p>
      </div>
    </div>
  )
}
